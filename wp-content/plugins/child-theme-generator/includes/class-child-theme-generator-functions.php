...
class Ch_Th_Gen_Functions {
	...
	*/
	public function child_themes_setup() {
		...
		<form method="post" action="" >
			...
			<fieldset>
				<table>
					<tr>
						<td class="FBLabel"><?php esc_html_e('Parent theme:', 'child-theme-generator');?></td>
						<td class="FBInput">
							<select name="parent">
								<?php
	...
	public function delete_theme_folder( $folder_to_remove, $parent_to_restore ) {
		...
		$response[$parent_to_restore] = "<p><span class='dashicons dashicons-yes'></span>" . esc_html__("Switching to ", "child-theme-generator") . str_repeat('&nbsp;', 1) .  "<b>$parent_to_restore</b> theme" . "</p>";
	} else {
